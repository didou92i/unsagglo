...
const SondageForm = ({ initial, onSubmit, onCancel }: SondageFormProps): JSX.Element => {
  const [data, setData] = useState<SondageFormData>(initial ?? DEFAULT);
  const [submitting, setSubmitting] = useState(false);

  const setField = <K extends keyof SondageFormData>(key: K, val: SondageFormData[K]): void =>
  ...
  const valid = data.question.trim().length > 0 && data.options.filter((o) => o.trim()).length >= 2;

  const handleSubmit = async (): Promise<void> => {
    if (!valid) return;
    setSubmitting(true);
    await onSubmit({ ...data, options: data.options.filter((o) => o.trim()) });
    setSubmitting(false);
  };
  ...
  return (
    <div className="space-y-4 rounded-lg border border-border p-4">
      <div>
        ...
        <Textarea value={data.question} onChange={(e) => setField("question", e.target.value)} placeholder="Quelle est votre question ?" />
      </div>
      <div>
        <Label>Theme</Label>
      ...
      <div className="flex items-center gap-2">
        <Switch checked={data.actif} onCheckedChange={(v) => setField("actif", v)} />
        <Label>Actif</Label>
      </div>
      <div className="space-y-2">
        ...
        {data.options.length < 6 && (
          <Button variant="outline" size="sm" onClick={addOption}><Plus className="mr-1 h-4 w-4" /> Ajouter</Button>
        )}
      </div>
      <div className="flex gap-2 pt-2">
        <Button onClick={handleSubmit} disabled={!valid || submitting}>{initial ? "Modifier" : "Creer"}</Button>
